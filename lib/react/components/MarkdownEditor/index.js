...
class MarkdownEditor extends React.Component {
	...
	render() {
		const { title, content, disabled, hideHelp, hidePreview } = this.props;
			...
			<div className='markdown editor'>
				<Tabs2 
					...
					<MediaQuery maxWidth={bp.xs}>
						<div className='heading'>{title ? title : 'Editor'}</div>
					</MediaQuery>
					<MediaQuery minWidth={bp.xs + 1}>
						<div className='heading'>{title ? title : 'Markdown Editor'}</div>
					</MediaQuery>

					<Tab2 id='editor' title='Editor' panel={
						<MarkdownInput src={content || this.state.content} onChange={onChange} readOnly={readOnly} disabled={disabled}/>
					}/>

					{hidePreview ? null :
						<Tab2 id='renderer' title='Preview' panel={
							<MarkdownRenderer className='markdown-content' src={content || this.state.content}/>
						}/> }
